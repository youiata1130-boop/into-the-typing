...
function swimmingGame(reducedMotion = false, type = "medaka_level_1") {
...
for (const type of ["medaka_level_1", "medaka_boss"]) {
  test(type + ": swimming loops through all 16 frames and does not restart while resting", () => {
  ...
  test(type + ": damage and enemy attacks return to swimming without leaking frame timers", () => {
    const game = swimmingGame(false, type);
  ...
  test(type + ": dialogue and ending a battle stop swimming; equipping the branch resumes it", () => {
    const game = swimmingGame(false, type);
  ...
  test(type + ": reduced motion keeps a still fish even when attacks finish", () => {
    const game = swimmingGame(true, type);
    assert.equal(game.run("window.swimIntervals.size"), 0);
