...
- Tap a kana key for its center letter; flick left, up, right, or down for the other letters. Holding a key shows its available letters and highlights the selected direction. The わ key offers を on the left, ん above, and ー on the right.
- Use ゛ for dakuten, ゜ for handakuten, and 小 for a small kana. These buttons modify the last letter; ⌫ deletes it. They are disabled when the current text cannot use them.
- Wrong kana immediately record a miss and disappear, keeping the correct prefix. A base kana can wait for the expected dakuten, handakuten, or small-kana button. Completing a word attacks immediately and clears the input for the next word.
- Input is committed once on pointer release. Canceled gestures, additional simultaneous touches, and gestures from a previous word or battle cannot enter text. Rotation and focus loss cancel an unfinished gesture.
...
- Validation: automated integration tests enter all 223 playable prompts using the game keys, including dakuten and small kana. They also cover typo rejection, deletion, clearing between words, gesture direction/cancellation, dialogue, and desktop behavior. Existing matcher and delayed IME-event regression tests remain. Browser checks use actual touch events at mobile sizes; physical iPhone Safari behavior still requires device verification.
